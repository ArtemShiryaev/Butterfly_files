...
private:
    ...
    void init(make_arg const& arg, Tail ... tail)
    {
        add_arg(arg);
        init(tail...);
    }
...
public:
    ...
    make_arg_list(make_arg const& first, Args ... args)
    {
        program_name = "program";
        init(first, args...);
    }
    ...
    std::map<std::string, std::string> parse(int argc, char const** argv)
    {
        ...
        for (int i = 1; i < argc; ++i)
        {
            ...
            if (std::get<1>(ans) == "")
            {
                ...
                if (args_map.find(key) == args_map.end())
                {
                    ...
                    key = "default-" + std::to_string(default_idx);
                    if (args_map.find(key) == args_map.end())
        ...
        for (auto const& e : args_map)
        {
            if (e.second->required)
            {
                if (result.find(e.second->name) == result.end())
                    throw_invalid_argument("property '", e.second->name, "' is not specified");
            }
        }
