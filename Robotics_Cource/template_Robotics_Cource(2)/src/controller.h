...
static double get_torque(double theta, double dtheta, double t)
{
	...
	//auto tau = Kp * ((_PI/2 + sin(4 * t)) - theta) + Kd * (4 * cos(4 * t) - dtheta) + 0.0022 * sign(dtheta) + 0.0000928 * dtheta;

	//auto tau = Kd * (0.1 - dtheta);

    //auto tau = Kd * (a - dtheta);
    auto tau = 0.0003 * t;
    //auto tau = Kp*(_PI/2 - theta) - Kd * dtheta;

    //auto tau = Kp*((_PI/2+sin(1.2*t)) - theta) + Kd * (1.2*cos(1.2*t) - dtheta);
    return tau;
}
